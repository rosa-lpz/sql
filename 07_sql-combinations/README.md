...
| Excpt | SELECT productCode, productName FROM products **EXCEPT** SELECT productCode, productName FROM products WHERE productLine = 'Classic Cars';|Returns products EXCEPT the Classic Cars product line, demonstrating how EXCEPT removes rows from the first result that appear in the second result.|
